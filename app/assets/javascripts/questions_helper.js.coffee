window.QuestionsHelper = class QuestionsHelper
  ...
  create_table: (question, root_section) ->
    @$questions_container.append(
      """
        ...
        <thead><tr><th class="title">Title</th><th>Type</th></tr></thead>
        <tbody><tr><td class="title">#{question.title}</td><td>#{question.type}</td></tr></tbody>
        </table><br>
      """
    )
  ...
  add_to_table: (question, root_section) ->
    $("##{@format_section_name(root_section)} tbody").append(
      """
      <tr><td class="title">#{question.title}</td><td>#{question.type}</td></tr>
      """
    )
